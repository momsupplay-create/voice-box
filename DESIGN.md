...
서비스 "우리 동네 목소리함"의 홈 화면은 [home-시안.html](home-시안.html)의 세 시안 중 **안 2(밀도형)**로 확정됐다 — 정보 밀도 높게, 사진은 작은 정사각 썸네일로, 히어로는 강한 한 줄 카피("말하면, 동네가 바뀝니다.")로 간다. 이때 CTA·필터 선택 버튼에 한해 강조색(오렌지)을 추가로 채택했다.
